...
fun KSAnnotated.isAnnotationPresent(qualifiedName: String): Boolean = isAnnotationPresent(qualifiedName.substring(qualifiedName.lastIndexOf(".")), qualifiedName)

fun KSAnnotated.isAnnotationPresent(simpleName: String, qualifiedName: String): Boolean = this.annotations.any {
    it.shortName.getShortName() == simpleName && it.annotationType.resolve().declaration.qualifiedName?.asString() == qualifiedName
}


fun KSAnnotated.isAnnotationPresentSimple(importInfo: ImportInfo): Boolean = isAnnotationPresentSimple(importInfo.simpleName)

fun KSAnnotated.isAnnotationPresentSimple(simpleName: String): Boolean = this.annotations.any { it.shortName.getShortName() == simpleName }
